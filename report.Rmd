...
Given either of these questions, we want to know if we can answer these questions using only 3 samples from each site in 2025, rather than 5 samples from each site. We will run 3 models for each of these questions; one model per GHG. We will drop sites ON15 until samples are updated.
...
co2_mod1_plot <- bayesplot::mcmc_intervals(co2_mod1$draws(c("mu", "alpha", "beta", "gamma")))
...
ch4_mod1_plot <- bayesplot::mcmc_intervals(ch4_mod1$draws(c("mu", "alpha", "beta", "gamma"))) +
...
n2o_mod1_plot <- bayesplot::mcmc_intervals(n2o_mod1$draws(c("mu", "alpha", "beta", "gamma"))) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank())
```
...
co2_mod2_plot <- bayesplot::mcmc_intervals(co2_mod2$draws(c("mu", "alpha", "beta", "gamma")))
...
ch4_mod2_plot <- bayesplot::mcmc_intervals(ch4_mod2$draws(c("mu", "alpha", "beta", "gamma"))) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank())
```
...
n2o_mod2_plot <- bayesplot::mcmc_intervals(n2o_mod2$draws(c("mu", "alpha", "beta", "gamma"))) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank())
```
